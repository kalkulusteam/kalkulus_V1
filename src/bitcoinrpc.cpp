// 
// 
// Copyright (c) 2017 Kalkulus Team Developers
...
int64 AmountFromValue(const Value& value)
{
    ...
    if (dAmount <= 0.0 || dAmount > 67000000.0)		// 67 million KLK supply
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "stop\n"
            "Stop Kalkulus server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Kalkulus server has now stopped running!";
}
...
// Kalkulus: Return average network hashes per second based on last number of blocks.
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateKalkuluss(fGenerate, pwalletMain);
    return Value::null;
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Kalkulus address for receiving payments.  "
    ...
    return CKalkulusAddress(keyID).ToString();
}


CKalkulusAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CKalkulusAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Kalkulus address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <kalkulusaddress> <account>\n"
            "Sets the account associated with the given address.");

    CKalkulusAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Kalkulus address");
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CKalkulusAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CKalkulusAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <kalkulusaddress> <amount> [comment] [comment-to] [tx-comment]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <kalkulusaddress> <message>\n"
    ...
    CKalkulusAddress addr(strAddress);
    if (!addr.IsValid())
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <kalkulusaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <kalkulusaddress> [minconf=1]\n"
            "Returns the total amount received by <kalkulusaddress> in transactions with at least [minconf] confirmations.");

    // Kalkulus address
    CKalkulusAddress address = CKalkulusAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Kalkulus address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <tokalkulusaddress> <amount> [minconf=1] [comment] [comment-to] [tx-comment]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());

    string strAccount = AccountFromValue(params[0]);
    CKalkulusAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid Kalkulus address");
    int64 nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CKalkulusAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CKalkulusAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid Kalkulus address:")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a Kalkulus address or hex-encoded public key\n"
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: Kalkulus address and we have full public key:
        CKalkulusAddress address(ks);
        if (address.IsValid())
        {
            CKeyID keyID;
    ...
    return CKalkulusAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CKalkulusAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CKalkulusAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CKalkulusAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CKalkulusAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CKalkulusAddress(r.first).ToString()));
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Kalkulus server stopping, restart to run with encrypted wallet";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
            a.push_back(CKalkulusAddress(addr).ToString());
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <kalkulusaddress>\n"
            "Return information about <kalkulusaddress>.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Kalkulus is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Kalkulus is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // Kalkulus: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: kalkulus-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
//
// JSON-RPC protocol.  Kalkulus speaks version 1.0 for maximum compatibility,
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use kalkulusd";
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 14320));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "14320")))
